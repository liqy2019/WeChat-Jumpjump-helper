...
public class JumpJumpHelper {
	...
	private final String[] ADB_SCREEN_CAPTURE_CMDS = { "adb shell /system/bin/screencap -p " + path + IMAGE_NAME,
			"adb pull " + path + "current.png " + STORE_DIR };

	// 截屏中游戏分数显示区域最下方的Y坐标，300是 1920x1080的值，根据实际情况修改
	private final int gameScoreBottomY = 500;

	// 按压的时间系数，可根据具体情况适当调节
	private final double pressTimeCoefficient = 1.36;

	// 按压的起始点坐标，也是再来一局的起始点坐标
	private final int swipeX = 650;

	private final int swipeY = 1636;
			...
			for (int y = gameScoreBottomY; y < height; y++) {
				...
				for (int x = 0; x < width; x++) {
					...
					if ((Math.abs(pixelR - lastPixelR) + Math.abs(pixelG - lastPixelG)
						...
						if(pixelR>=122 && pixelR<=126 && pixelG>=140 && pixelG<=142 &&
								pixelB>=156 && pixelB<=158) {continue;}
						boardXSum += x;
						boardXCount++;
					}
				}
